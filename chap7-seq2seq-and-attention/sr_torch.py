...
class Seq2SeqNoAttention(nn.Module):
    ...
    def forward(self, enc_ids: torch.Tensor, dec_ids: torch.Tensor) -> torch.Tensor:
        ...
        dec_out, _ = self.decoder(dec_emb, h)
    ...
    @torch.no_grad()
    def get_next_token(self, token: torch.Tensor, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        emb = self.embedding(token).unsqueeze(1)  # [b, 1, emb]
        dec_out, state = self.decoder(emb, state)
        logits = self.output(dec_out.squeeze(1))
        next_token = torch.argmax(logits, dim=-1)
        return next_token, state


def train(model: Seq2SeqNoAttention, steps: int, seqlen: int, batch_size: int, lr: float, device: torch.device) -> None:
...
@torch.no_grad()
def sequence_reversal(model: Seq2SeqNoAttention, batch_size: int, length: int, device: torch.device) -> Tuple[List[str], List[str]]:
    model.eval()
...
def main() -> None:
    ...
    if device.type == "cuda":
        ...
        print("use cpu")
    model = Seq2SeqNoAttention().to(device)
